...
// GetAuroraMetricsURLHandler gets RDS aurora metrics from cloudwatch and returns a link to the image
func (s *server) GetAuroraMetricsURLHandler(w http.ResponseWriter, r *http.Request) {
	w = LogWriter{w}
	vars := mux.Vars(r)
	account := vars["account"]
	instanceId := vars["id"]
